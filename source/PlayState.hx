...
class PlayState extends MusicBeatState
{
	...
	public static var WeedRonisDone:Bool = false;
	...
	override public function create()
	{
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'certified-champion' | 'rong-aisle':
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			var creds:FlxSprite  = new FlxSprite().loadGraphic(Paths.image('creds/${SONG.song.toLowerCase()}'));
			creds.setGraphicSize(Std.int(creds.width * .85));
			creds.x -= creds.width;
			creds.screenCenter(Y);
			creds.visible = true;
			creds.antialiasing = true;
			creds.scrollFactor.set();
			creds.cameras = [camHUD];
			FlxTween.tween(creds, {x: 25}, 1, {
				ease: FlxEase.quartInOut,
				startDelay: 1,
				onComplete: function(twn:FlxTween)
				{
					FlxTween.tween(creds, {x: -creds.width}, 1, {
						ease: FlxEase.quartInOut,
						startDelay: 1.75,
						onComplete: function(twn:FlxTween)
						{
	...
	override public function update(elapsed:Float)
	{
		...
		if (Paths.formatToSongPath(SONG.song) == 'rong-aisle' && isStoryMode)
		{
			FreeplayState.WeedRon = true;
			FlxG.save.data.WeedRon = FreeplayState.WeedRon;
			FlxG.save.flush();
		}

		if (FlxG.save.data.WeedRonisDone != null)
		{
			WeedRonisDone = FlxG.save.data.WeedRonisDone;
		}
		
		if (Paths.formatToSongPath(SONG.song) == 'weed')
		{
			WeedRonisDone = true;
			FlxG.save.data.WeedRonisDone = WeedRonisDone;
			FlxG.save.flush();
		}
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				{
					FlxG.sound.playMusic(Paths.music('ron_menu'));

					cancelMusicFadeTween();
					if(FlxTransitionableState.skipNextTransIn) {
						CustomFadeTransition.nextCamera = null;
					}
					MusicBeatState.switchState(new COOLWEEDRONState());

					if(FlxG.save.data.WeedRonisDone)
