...
class WeekEditorState extends MusicBeatState
{
	...
	function reloadWeekThing() {
		...
		var isMissing:Bool = true;
		if(assetName != null && assetName.length > 0) {
			if( #if MODS_ALLOWED FileSystem.exists(Paths.modsImages('storyshit/' + assetName)) || #end
			Assets.exists(Paths.getPath('images/storyshit/' + assetName + '.png', IMAGE), IMAGE)) {
				weekThing.loadGraphic(Paths.image('storyshit/' + assetName));
				isMissing = false;
			}
		}
		...
		if(isMissing) {
			...
			missingFileText.text = 'MISSING FILE: images/storyshit/' + assetName + '.png';
		}
