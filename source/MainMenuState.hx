...
class MainMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var bg:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('coolguymcgee'));
		...
		var yScroll:Float = Math.max(0.25 - (0.05 * (optionShit.length - 4)), 0.1);
		ron = new FlxSprite(-80).loadGraphic(Paths.image('menu_rons/ron_' + FlxG.random.int(0, 12)));
		ron.scrollFactor.set(0, yScroll);
		ron.setGraphicSize(Std.int(ron.width * 1.175 ));
		ron.alpha = 0.1;
		ron.updateHitbox();
		ron.screenCenter();
		...
		add(versionShit);
		var versionShit:FlxText = new FlxText(12, FlxG.height - 24, 0, "ron v" + Application.current.meta.get('version'), 12);
		versionShit.scrollFactor.set();
		versionShit.setFormat("VCR OSD Mono", 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(versionShit);
